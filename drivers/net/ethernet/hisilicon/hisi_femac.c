...
static int hisi_femac_drv_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < CLK_NUM; i++) {
		priv->clks[i] = devm_clk_get_optional(&pdev->dev, clk_strs[i]);
		if (IS_ERR(priv->clks[i])) {
			dev_err(dev, "failed to get clk %s: %ld\n", clk_strs[i], PTR_ERR(priv->clks[i]));
			ret = -ENODEV;
			goto out_free_netdev;
		}
	}
	...
	for (i = 0; i < CLK_NUM; i++) {
		...
		if (ret) {
			dev_err(dev, "failed to enable clk %s: %d\n", clk_strs[i], ret);
			goto out_free_netdev;
		}
	}
