...
#define SDMMC_ENABLE_NEW_TUNING	BIT(0)
...
static int dw_mci_hi3798mv200_endisable_tuning(struct dw_mci_slot *slot, bool enable)
{
	struct dw_mci *host = slot->host;
	struct dw_mci_histb_priv *priv = host->priv;
	int ret;
	...
	val = mci_readl(host, TUNING_CTRL);
	if (enable) {
		ret = hisi_sdmmc_dll_enable_tuning(priv->sample_clk);
		if (ret)
			return ret;

		val |= SDMMC_ENABLE_NEW_TUNING;
	} else {
	...
	mci_writel(host, TUNING_CTRL, val);

	return 0;
}

static inline int dw_mci_hi3798mv200_enter_tuning(struct dw_mci_slot *slot)
{
	return dw_mci_hi3798mv200_endisable_tuning(slot, true);
}

static inline int dw_mci_hi3798mv200_exit_tuning(struct dw_mci_slot *slot)
{
	return dw_mci_hi3798mv200_endisable_tuning(slot, false);
}
					     ...
					     u32 opcode)
{
	...
	// enable tuning
	ret = dw_mci_hi3798mv200_enter_tuning(slot);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(degrees); i++) {
		clk_set_phase(priv->sample_clk, degrees[i]);
...
tuning_out:
	// disable tuning
	ret = dw_mci_hi3798mv200_exit_tuning(slot);
	if (ret)
		return ret;
	if (found) {
		if (raise_point == -1)
